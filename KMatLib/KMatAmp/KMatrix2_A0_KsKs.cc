...
#include "KMatrix2_A0_KsKs.h"


KMatrix2_A0_KsKs::KMatrix2_A0_KsKs(const vector<string> &args): UserAmplitude<KMatrix2_A0_KsKs>(args) {
    /*
     * Usage: KMatrix2_A0_KsKs <daughter 1> <daughter 2> <Re[f_0(400)]> <Im[f_0(400)]> ...
     */
...
void KMatrix2_A0_KsKs::calcUserVars(GDouble** pKin, GDouble* userVars) const {
    ...
    // Loop over channels
    for (int j = 0; j < 2; j++) {
        mat_C(j, j) = KMatrix2_A0_KsKs::chew(s, m1s[j], m2s[j]);
    }
    ...
    SMatrix2 temp_inv = KMatrix2_A0_KsKs::inverse2(temp);
...
complex<GDouble> KMatrix2_A0_KsKs::calcAmplitude(GDouble** pKin, GDouble* userVars) const {
...
SMatrix2 KMatrix2_A0_KsKs::inverse2(SMatrix2 A) const {
    SMatrix2 M;
...
complex<GDouble> KMatrix2_A0_KsKs::rho(double s, double m1, double m2) const {
    return sqrt(complex<GDouble>(((1 - ((m1 + m2) * (m1 + m2) / s)) * (1 - ((m1 - m2) * (m1 - m2) / s))), 0));
}

complex<GDouble> KMatrix2_A0_KsKs::xi(double s, double m1, double m2) const {
    return complex<GDouble>(1 - ((m1 + m2) * (m1 + m2) / s), 0);
}

complex<GDouble> KMatrix2_A0_KsKs::chew(double s, double m1, double m2) const {
    complex<GDouble> tot = 0;
    tot += (KMatrix2_A0_KsKs::rho(s, m1, m2) / Pi()) * log((KMatrix2_A0_KsKs::xi(s, m1, m2) + KMatrix2_A0_KsKs::rho(s, m1, m2)) / (KMatrix2_A0_KsKs::xi(s, m1, m2) - KMatrix2_A0_KsKs::rho(s, m1, m2)));
    tot -= (KMatrix2_A0_KsKs::xi(s, m1, m2) / Pi()) * ((m2 - m1) / (m1 + m2)) * log(m2 / m1);
    return tot;
}

void KMatrix2_A0_KsKs::updatePar(const AmpParameter &par) {}
