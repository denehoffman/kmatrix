...
void KMatrix2_Pi1::calcUserVars(GDouble** pKin, GDouble* userVars) const {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        temp_K *= KMatrix2_Pi1::poleProductRemainder(s, i);
        // Loop over channels: 
        SVector2 B_factor;
        for (int j = 0; j < 2; j++) {
    ...
    SMatrix2 temp;
    complex<GDouble> product = KMatrix2_Pi1::poleProduct(s);
    for (int i = 0; i < 2; ++i) {
        temp(i, i) = product;
    }
    mat_K *= mat_C;
...
complex<GDouble> KMatrix2_Pi1::calcAmplitude(GDouble** pKin, GDouble* userVars) const {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        temp_P = couplings[i];
        temp_P *= betas[i]; 
        temp_P *= KMatrix2_Pi1::poleProductRemainder(s, i);
        // Loop over channels:
        SVector2 B_factor;
        for (int j = 0; j < 2; j++) {
            GDouble q_alpha = fabs(breakupMomentum(masses[i], m1s[j], m2s[j]));
...
complex<GDouble> KMatrix2_Pi1::poleProduct(double s) const {
...
complex<GDouble> KMatrix2_Pi1::poleProductRemainder(double s, size_t index) const {
    complex<GDouble> tot = 1;
    for (size_t i = 0; i < masses.size(); ++i) {
        if (i == index) continue;
        tot *= complex<GDouble>(masses[i] * masses[i] - s);
