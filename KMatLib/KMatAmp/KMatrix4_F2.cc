...
KMatrix4_F2::KMatrix4_F2(const vector<string> &args): UserAmplitude<KMatrix4_F2>(args) {
    /*
     * Usage: KMatrix4_F2 <daughter 1> <daughter 2> <channel> <Re[f_0(400)]> <Im[f_0(400)]> ...
     */
...
void KMatrix4_F2::calcUserVars(GDouble** pKin, GDouble* userVars) const {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        temp_K *= KMatrix4_F2::poleProductRemainder(s, i);
        // Loop over channels: 
        SVector4 B_factor;
        for (int j = 0; j < 4; j++) {
    ...
    SMatrix4 temp;
    complex<GDouble> product = KMatrix4_F2::poleProduct(s);
    for (int i = 0; i < 4; ++i) {
        temp(i, i) = product;
    }
    mat_K *= mat_C;
...
complex<GDouble> KMatrix4_F2::calcAmplitude(GDouble** pKin, GDouble* userVars) const {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        temp_P *= KMatrix4_F2::poleProductRemainder(s, i);
        // Loop over channels:
        SVector4 B_factor;
        for (int j = 0; j < 4; j++) {
            GDouble q_alpha = fabs(breakupMomentum(masses[i], m1s[j], m2s[j]));
...
complex<GDouble> KMatrix4_F2::poleProduct(double s) const {
...
complex<GDouble> KMatrix4_F2::poleProductRemainder(double s, size_t index) const {
    complex<GDouble> tot = 1;
    for (size_t i = 0; i < masses.size(); ++i) {
        if (i == index) continue;
        tot *= complex<GDouble>(masses[i] * masses[i] - s);
