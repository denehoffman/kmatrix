...
#include "KMatrix4_F2_KsKs.h"


KMatrix4_F2_KsKs::KMatrix4_F2_KsKs(const vector<string> &args): UserAmplitude<KMatrix4_F2_KsKs>(args) {
    /*
     * Usage: KMatrix4_F2_KsKs <daughter 1> <daughter 2> <Re[f_0(400)]> <Im[f_0(400)]> ...
     */
...
void KMatrix4_F2_KsKs::calcUserVars(GDouble** pKin, GDouble* userVars) const {
    ...
    // Loop over channels
    for (int j = 0; j < 4; j++) {
        mat_C(j, j) = KMatrix4_F2_KsKs::chew(s, m1s[j], m2s[j]);
    }
    ...
    SMatrix4 temp_inv = KMatrix4_F2_KsKs::inverse4(temp);
...
complex<GDouble> KMatrix4_F2_KsKs::calcAmplitude(GDouble** pKin, GDouble* userVars) const {
...
SMatrix4 KMatrix4_F2_KsKs::inverse4(SMatrix4 A) const {
    SMatrix4 M;
...
complex<GDouble> KMatrix4_F2_KsKs::rho(double s, double m1, double m2) const {
    return sqrt(complex<GDouble>(((1 - ((m1 + m2) * (m1 + m2) / s)) * (1 - ((m1 - m2) * (m1 - m2) / s))), 0));
}

complex<GDouble> KMatrix4_F2_KsKs::xi(double s, double m1, double m2) const {
    return complex<GDouble>(1 - ((m1 + m2) * (m1 + m2) / s), 0);
}

complex<GDouble> KMatrix4_F2_KsKs::chew(double s, double m1, double m2) const {
    complex<GDouble> tot = 0;
    tot += (KMatrix4_F2_KsKs::rho(s, m1, m2) / Pi()) * log((KMatrix4_F2_KsKs::xi(s, m1, m2) + KMatrix4_F2_KsKs::rho(s, m1, m2)) / (KMatrix4_F2_KsKs::xi(s, m1, m2) - KMatrix4_F2_KsKs::rho(s, m1, m2)));
    tot -= (KMatrix4_F2_KsKs::xi(s, m1, m2) / Pi()) * ((m2 - m1) / (m1 + m2)) * log(m2 / m1);
    return tot;
}

void KMatrix4_F2_KsKs::updatePar(const AmpParameter &par) {}
