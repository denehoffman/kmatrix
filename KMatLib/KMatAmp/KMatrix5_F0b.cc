...
#include "KMatrix5_F0b.h"


KMatrix5_F0b::KMatrix5_F0b(const vector<string> &args): UserAmplitude<KMatrix5_F0b>(args) {
    /*
     * Usage: KMatrix5_F0b <daughter 1> <daughter 2> <Re[f_0(500)]> <Im[f_0(500)]> ...<Re[c0]> <Im[c0]>
     */
    ...
    c0_re = AmpParameter(args[12]);
    c0_im = AmpParameter(args[13]);
    ...
    registerParameter(c0_re);
    registerParameter(c0_im);
...
void KMatrix5_F0b::calcUserVars(GDouble** pKin, GDouble* userVars) const {
    ...
    // Loop over channels
    for (int j = 0; j < 5; j++) {
        mat_C(j, j) = KMatrix5_F0b::chew(s, m1s[j], m2s[j]);
    }
    ...
    SMatrix5 temp_inv = KMatrix5_F0b::inverse5(temp);
...
complex<GDouble> KMatrix5_F0b::calcAmplitude(GDouble** pKin, GDouble* userVars) const {
    ...
    for (int i = 0; i < 5; i++) {
        SVector5 temp_P;
        SMatrix5 temp_B; // TODO: Probably don't need this
        temp_P = couplings[i];
        temp_P *= betas[i]; 
        GDouble denominator = (masses[i] * masses[i] - s);
        // if (masses[i] == m) {denominator += 1E-3;} // just in case
        ...
        temp_P /= denominator;
        // add background
        temp_P[2] += complex<GDouble>(c0_re, c0_im);
        // Loop over channels:
        SVector5 B_factor;
        for (int j = 0; j < 5; j++) {
            GDouble q_alpha = fabs(breakupMomentum(masses[i], m1s[j], m2s[j]));
        ...
        temp_B = TensorProd(B_factor, B_factor); // TODO: I don't think I need this line
        temp_P = temp_P * B_factor;
        vec_P += temp_P;
    }
    SMatrix5 temp_inv; // TODO: We actually only need some of these?
    for (int i = 0; i < 5; i++) {
        for (int j = 0; j < 5; j++) {
...
SMatrix5 KMatrix5_F0b::inverse5(SMatrix5 A) const {
    SMatrix5 M;
...
complex<GDouble> KMatrix5_F0b::rho(double s, double m1, double m2) const {
    return sqrt(complex<GDouble>(((1 - ((m1 + m2) * (m1 + m2) / s)) * (1 - ((m1 - m2) * (m1 - m2) / s))), 0));
}

complex<GDouble> KMatrix5_F0b::xi(double s, double m1, double m2) const {
    return complex<GDouble>(1 - ((m1 + m2) * (m1 + m2) / s), 0);
}

complex<GDouble> KMatrix5_F0b::chew(double s, double m1, double m2) const {
    complex<GDouble> tot = 0;
    tot += (KMatrix5_F0b::rho(s, m1, m2) / Pi()) * log((KMatrix5_F0b::xi(s, m1, m2) + KMatrix5_F0b::rho(s, m1, m2)) / (KMatrix5_F0b::xi(s, m1, m2) - KMatrix5_F0b::rho(s, m1, m2)));
    tot -= (KMatrix5_F0b::xi(s, m1, m2) / Pi()) * ((m2 - m1) / (m1 + m2)) * log(m2 / m1);
    return tot;
}

void KMatrix5_F0b::updatePar(const AmpParameter &par) {}
