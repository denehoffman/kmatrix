...
Ylm::Ylm(const vector<string> &args) : UserAmplitude<Ylm>(args) {
  assert(args.size() == 3);
...
complex<GDouble> Ylm::calcAmplitude(GDouble **pKin) const {
  ...
  return complex<GDouble>(static_cast<GDouble>(m_phaseFactor) *
                          Y(m_j, m_m, cosTheta, phi));
}
