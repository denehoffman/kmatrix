...
#define S3J_MAX(a, b, c, ris)                                                  \
  (((a) > (b) ? (ris = (a)) : (ris = (b))) > (c) ? ris : (ris = (c)))
#define S3J_MIN(a, b, c, ris)                                                  \
  (((a) < (b) ? (ris = (a)) : (ris = (b))) < (c) ? ris : (ris = (c)))
...
* | -------------------------------------- ...
  ...
  *    |   ---------------------------------------------------------------------
  ...
  j1+j2-j3   >= 0		j1-j2+j3   >= 0		-j1+j2+j3 >= 0
j1+j2+j3+1 >= 0 k          >= 0		j1+j2-j3-k >= 0 j1-m1-k   >= 0
j2+m2-k    >= 0 j3-j2+m1+k >= 0		j3-j1-m2+k >= 0
  ...
  and k values in the sum must be such that
                          ...
                          k <= j1+j2-j3		(4)			k >= 0
(7) k <= j1-m1			(5)			k >= -j3+j2-m1
(8) k <= j2+m2			(6)			k >= -j3+j1+m2
(9)

  If no values of k satisfy the (4) to (9), the result is null because the sum
is null, otherwise one can find   kmin < kmax   such that
  ...
  Once the values of kmin and kmax are found, the only "selection rule" is
kmin<kmax.
  */
  ...
  jpm1 = (int)(j1 + m1);
  if (!S3J_EQUAL(jpm1, j1 + m1))
    return 0.0;

  jpm2 = (int)(j2 + m2);
  if (!S3J_EQUAL(jpm2, j2 + m2))
    return 0.0;

  jpm3 = (int)(j3 + m3);
  if (!S3J_EQUAL(jpm3, j3 + m3))
    return 0.0;

  jmm1 = (int)(j1 - m1);
  if (!S3J_EQUAL(jmm1, j1 - m1))
    return 0.0;

  jmm2 = (int)(j2 - m2);
  if (!S3J_EQUAL(jmm2, j2 - m2))
    return 0.0;

  jmm3 = (int)(j3 - m3);
  if (!S3J_EQUAL(jmm3, j3 - m3))
    return 0.0;
  ...
  for (k = kmin; k <= kmax; ++k) {
    ...
    ris += mult / (f[k] * f[j1pj2mj3 - k] * f[jmm1 - k] * f[jpm2 - k] *
                   f[j3mj2pm1 + k] * f[j3mj1mm2 + k]);
