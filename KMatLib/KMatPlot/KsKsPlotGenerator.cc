...
KsKsPlotGenerator::KsKsPlotGenerator(const FitResults &results)
    : PlotGenerator(results) {
  bookHistogram(kKsKsMass, new Histogram1D(100, 1.0, 2.0, "ksksMass",
                                           "Invariant Mass of K_{S}K_{S}"));
  bookHistogram(kKsKsMassFine, new Histogram1D(200, 1.0, 2.0, "ksksMassFine",
                                               "Invariant Mass of K_{S}K_{S}"));
  bookHistogram(kKsKsMassCourse,
                new Histogram1D(40, 1.0, 2.0, "ksksMass",
                                "Invariant Mass of K_{S}K_{S}"));
  bookHistogram(kCosTheta,
                new Histogram1D(50, -1., 1., "cosTheta",
                                "cos(#theta_{HX}) of Resonance Production"));
  bookHistogram(kPhi, new Histogram1D(50, -180, 180, "Phi", "#phi_{HX}"));
  bookHistogram(kt, new Histogram1D(100, 0, 2.00, "t", "-t"));
  bookHistogram(kCosThetaVMass,
                new Histogram2D(100, 1.0, 2.0, 50, -1., 1., "cosThetaVMass",
                                "cos(#theta_{HX}) vs. m(K_{S}K_{S})"));
  bookHistogram(kPhiVMass,
                new Histogram2D(100, 1.0, 2.0, 50, -180, 180, "phiVMass",
                                "#phi_{HX} vs. m(K_{S}K_{S})"));
}
...
void KsKsPlotGenerator::projectEvent(Kinematics *kin) {
  ...
  fillHistogram(kKsKsMassCourse, resonance.M());
